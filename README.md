...
- 🎬 **预热加载页** — 6 套创意风格（极光光幕/影院倒计时/霓虹灯管/舞台聚光/深空跃迁/水墨晕染），每套独立构图与动效，全屏接管、替换 Emby 默认黑屏 logo 启动页；图标支持 AI 设计 / Emby 原生 / 自定义图片 三选
- 🖥️ **网页设置中心** — 部署后在页面内实时换主题、自定义颜色、开关渐变、**换扁平 Logo 预设**，零改代码
- 🏷️ **扁平 Logo 预设库** — 内置极光/Emby/极简/影院/胶片 5 套扁平 Logo，安装或设置中心随时挑，给你定制主题的愉悦感
- 🖼️ **沉浸式首页轮播** — API 驱动，真正满屏 Backdrop，顶栏透明悬浮（Netflix 式），三种样式可选（沉浸满屏 / 玻璃信息条 / 极简），SPA 自适应
- 📋 **详情页增强** — 直接读元数据已有评分（敏感片也能显示）、剧照墙（拖动滑动 + 点击灯箱放大）、预告片、演职员、同类/同演员推荐 + IMDb/TMDB/TVDB/豆瓣外链；评分紧跟按钮组、剧照推荐放到简介之后，不挤占首屏
- ▶️ **外部播放器** — 点击弹出多协议下拉菜单，PotPlayer / VLC / mpv / IINA / 复制直链 一键调起，外观对齐 Emby 原生按钮（可选）
